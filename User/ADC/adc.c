...
{  
	int Volt;//电池电压
	Volt=Get_Adc(Battery_Ch)*1000*3.3/4096;	//电阻分压10：1
...
{  
	...
	ADC_RegularChannelConfig(ADC2, 8, 1, ADC_SampleTime_239Cycles5 );	//ADC1,ADC通道,采样时间为239.5周期	  			     
	ADC_SoftwareStartConvCmd(ADC2, ENABLE);		//使能指定的ADC1的软件转换启动功能		 
	while(!ADC_GetFlagStatus(ADC2, ADC_FLAG_EOC ));//等待转换结束
	Volt=ADC_GetConversionValue(ADC2)*3.3/4096;	//电阻分压10：1
		                                //=====读取电池电压		
	pj_j++;                                                    //=====平均值计数器
	j_zhi+=Volt;                                          //=====多次采样累积
	if(pj_j==4) 
	{
		Volt=j_zhi/4,j_zhi=0,pj_j=0; 	 	//=====求平均值	
